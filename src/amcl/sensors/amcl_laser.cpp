...
// note that pf_update_sensor in pf.c is modified by Chung-Ying
// there is no normalization in pf_update_sensor
double AMCLLaser::UpdateSensor(pf_t *pf, AMCLSensorData *data)
{
  if (this->max_beams < 2)
    return 0.0;
  double total = 0.0;
...
double AMCLLaser::BeamModel(AMCLLaserData *data, pf_sample_set_t* set)
{
  ...
  for (j = 0; j < set->sample_count; j++)
  {
    ...
    for (i = 0; i < data->range_count; i += step)
    {
      ...
      if(pz > 1.0)
      {
        printf("pz is larger than 1: %lf\n", pz);
        printf("current pose is: %lf, %lf, %lf\n", pose.v[0], pose.v[1], pose.v[2]);
        printf("z = obs_range - map_range: %lf = %lf - %lf\n", z, obs_range, map_range);
        printf("exp value: %lf\n",  self->z_hit * exp(-(z * z) / (2 * self->sigma_hit * self->sigma_hit)));
        printf("lamda value: %lf\n", self->z_short * self->lambda_short * exp(-self->lambda_short*obs_range));
        printf("max value when obs_range == data->range_max: %lf\n", self->z_max * 1.0);
        printf("random value when obs_range < daa->range_max: %lf\n", self->z_rand * 1.0/data->range_max);
        printf("data->range_max is: %lf\n", data->range_max);
        printf("z values are: %lf, %lf, %lf, %lf\n", self->z_hit, self->z_short, self->z_max, self->z_rand);
        pz = 1;
      }
      assert(pz <= 1.0);
      assert(pz >= 0.0);
